...
for RULESET in ${INPUT_TFLINT_RULESETS}; do
  ...
  curl -sfL "${REPOSITORY}"/releases/latest/download/"${PLUGIN}"_linux_$(uname -m | sed 's/x86_/amd/; s/aarch64/arm/').zip -o "${PLUGIN}".zip && unzip "${PLUGIN}".zip -d "${TFLINT_PLUGIN_DIR}" && rm "${PLUGIN}".zip
  echo '::endgroup::'
done
